        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center justify-between mb-12">
            <div className="flex flex-col items-center flex-1">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold mb-2 ${step !== 'email' ? 'bg-green-500 text-white' : 'bg-green-500 text-white'}`}>
                {step !== 'email' && step !== 'success' ? <CheckCircle size={24} /> : '1'}
              </div>
              <span className="text-xs font-semibold text-gray-700">Email</span>
            </div>

            <div className={`flex-1 h-1 mx-2 mb-8 ${['otp', 'password', 'success'].includes(step) ? 'bg-green-500' : 'bg-gray-300'}`}></div>

            <div className="flex flex-col items-center flex-1">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold mb-2 ${step !== 'otp' && step !== 'email' ? 'bg-green-500 text-white' : step === 'otp' ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-700'}`}>
                {step !== 'otp' && step !== 'email' && step !== 'success' ? <CheckCircle size={24} /> : '2'}
              </div>
              <span className="text-xs font-semibold text-gray-700">OTP</span>
            </div>

            <div className={`flex-1 h-1 mx-2 mb-8 ${['password', 'success'].includes(step) ? 'bg-green-500' : 'bg-gray-300'}`}></div>

            <div className="flex flex-col items-center flex-1">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold mb-2 ${step === 'success' ? 'bg-green-500 text-white' : step === 'password' ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-700'}`}>
              ...
              {otpSent && (
                <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    <p className="font-semibold text-green-900">Code sent successfully!</p>
                    <p className="text-green-700">Check your email at <span className="font-medium">{email}</span></p>
                  </div>
                </div>
              )}
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    <ul className="space-y-1 text-xs text-gray-600">
                      <li className={`flex items-center gap-2 ${newPassword.length >= 8 ? 'text-green-600' : ''}`}>
                        <span className={`w-4 h-4 rounded-full flex items-center justify-center ${newPassword.length >= 8 ? 'bg-green-500 text-white' : 'border border-gray-300'}`}>
